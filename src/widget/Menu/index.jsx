...
function ACMenu(props) {
  ...
  function onClickMenuItem(path) {
    ...
    const prefixArr =
      lang === 'zh-CN' ? pathArr.slice(0, 2) : pathArr.slice(0, 3);
    const currentRoute = flattenRoutes.find(
      (r) => `${prefixArr.join('/')}/${r.path}` === path
    );
  ...
  return (
    ...
    >
      <div
        id="menu"
        className="ac-menu"
        style={{ paddingTop: `${noticeHeight}px` }}
      >
        <MenuHeader />
                  ...
                  >
                    {group.children.map((child) => {
                        ...
                        return (
                          <MenuItemGroup key={child.key} title={child.name}>
                            {child.items.map((item) =>
                              !item.hide ? (
                                <MenuItem
                                  key={getPath(group.module, item.path, lang)}
                                >
                                  <a
                                    href={getPath(
                                      group.module,
                      ...
                      return (
                        !child.hide && (
                          <MenuItem
                            key={getPath(group.module, child.path, lang)}
                          >
                            <a
